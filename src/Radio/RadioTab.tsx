...
class RadioTab extends React.Component<RadioTabsPassedDownProps & RadioTabProps, {}> {
	...
	setId = (id: string) => {
		...
		if (!setId)
			throw new Error(
				`setId is not pass down from RadioTabs. please check RadioTab - ${id} is direct child of RadioTabs or create the wrapper to pass the props from RadioTabs`
			);
...
export default compose<RadioTabsPassedDownProps & RadioTabProps, RadioTabProps>(
	radioTab => radioTab
)(RadioTab);
