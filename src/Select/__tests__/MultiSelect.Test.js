...
describe('<MultiSelect /> mount', () => {
	...
	it('should set the labelPrefix', () => {
		expect(wrapper.find('.input-label-prefix').text() === allMultiSelectProps.labelPrefix).toBe(
			true
		);
	});

	it('should set the labelPostfix', () => {
		expect(wrapper.find('.input-label-postfix').text() === allMultiSelectProps.labelPostfix).toBe(
			true
		);
	});
