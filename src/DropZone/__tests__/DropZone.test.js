...
const filePreviewItem = (file, index) => (
	<p key={index} className="preview-item">
		{file.name}
	</p>
);
...
describe('DropZone', () => {
	...
	it('adds user defined classes to the dropzone', () => {
		...
		expect(extractedClassName(singleFileWrapper)).toBe(
			`drop-zone-box ${singleFileAllowedProps.className}`
		);
		expect(extractedClassName(multipleFileWrapper)).toBe(
			`drop-zone-box ${multipleFilesAllowedProps.className}`
		);
	});
	...
	it('should show file preview(s) if user has provided preview component', () => {
		const singleFileWrapper = shallow(
			<DropZoneUI
				{...singleFileAllowedProps}
				fileListComponent={filePreviewComponent}
				value={Set([droppableFiles[0]])}
			/>
		);
		const multipleFileWrapper = shallow(
	...
	it('should mount', () => {
		...
		const singleFileWrapper = mount(
			<DropZone {...singleFileAllowedProps} fileListComponent={filePreviewComponent} />,
			mountOptions
		);
		...
		const multipleFileWrapper = mount(
			<DropZone {...multipleFilesAllowedProps} fileListComponent={filePreviewComponent} />,
			mountOptions
		);
