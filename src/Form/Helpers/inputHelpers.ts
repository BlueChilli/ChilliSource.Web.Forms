...
export const getHTMLAttributes = <T extends GetHTMLAttributesGuard> (props:T) => {
  const safeProps = pick<PickedAttrs, T>(props, "id", "autoFocus", "required", "name", "type", "value", "min", "max", "minLength", "maxLength", "pattern", "accept", "multiple", "placeholder", "disabled", "rows", "style");
  return safeProps;
}