...
describe('returnCheckedValue()', () => {
	it('should return undefined when no valid argument is passed', () => {
		expect(
			returnCheckedValue(arg => !isUndefined(arg), undefined, undefined, undefined, undefined)
		).toBe(undefined);
	});
	it('should return first valid argument', () => {
		const firstValue = true;
		expect(returnCheckedValue(arg => !isUndefined(arg), undefined, firstValue, '', undefined)).toBe(
			firstValue
		);
	});
	it('should return first valid argument when it is last', () => {
		const firstValue = true;
		expect(
			returnCheckedValue(arg => !isUndefined(arg), undefined, undefined, undefined, firstValue)
		).toBe(firstValue);
	});
});
...
describe('getHTMLAttributes', () => {
	...
	it('excludes unsupported attributes', () => {
		const returnedSupportedProps = getHTMLAttributes(
			Object.assign({}, supportedProps, unsupportedProps)
		);
		expect(isEqual(supportedProps, returnedSupportedProps)).toBe(true);
	});
});
