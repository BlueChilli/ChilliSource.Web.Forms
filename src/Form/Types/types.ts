...
export type PossibleDefaultValues =
  | number
  | string
  | boolean
  | Moment
...
interface BaseInputProps<TDefault, TValue, TChangeEvent = ChangeEvent<{}>>
  extends BaseReactProps,
...
type compareAdditionalProps = <TProps>(
  props: TProps,
  nextProps: TProps
) => boolean;
...
export interface PerformanceWrapperWithProps
  extends InputInfoProps,
    DefaultValueProp<PossibleDefaultValues> {
  inputPath: string[];
}
