...
const mapOutput = (
	data: Map<string, any>,
	mapOutputFunc: ((data?: Map<string, any>) => Map<string, any>)
) => {
			...
			defer(() => {
				...
				if (firstError === null) {
					if (onSubmit) {
						...
						const normalizedFields = mapOutput(
							normalizeFields(fields) as Map<string, any>,
							this.props.mapOutput
						);
...
export default branch<FormProps<undefined>>(
	props => {
		return !(props.FormState && props.dispatch);
	},
	withReducer('FormState', 'dispatch', withReducerState, Map<string, {}>()) as any
)(
