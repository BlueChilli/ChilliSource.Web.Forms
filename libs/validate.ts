...
export function validationsAvailable<T>(inputAttributes: T) {
	...
	return validationsAvail.filter(
		validation => inputAttributes.hasOwnProperty(validation) && validation !== 'default'
	) as InputTest[];
}
		...
		if (typeof test === 'function') {
			...
			if (typeof customValidation === 'boolean' || customValidation === 'undefined') {
				...
				return console.error(
					'Custom validation functions must return a bool, undefined or a promise'
				);
			}
		}
